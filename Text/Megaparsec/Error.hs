...
data ParseError s e
  = TrivialError Int (Maybe (ErrorItem (Element (Tokens s)))) (Set (ErrorItem (Element (Tokens s))))
...
deriving instance ( Show (Element (Tokens s))
                  , Show e
                  ) => Show (ParseError s e)

deriving instance ( Eq (Element (Tokens s))
                  , Eq e
                  ) => Eq (ParseError s e)
...
instance ( NFData (Element (Tokens s))
         , NFData e
...
instance ( Show (Tokens s)
         , Show (Element (Tokens s))
         , Show e
...
data ParseErrorBundle s e = ParseErrorBundle
  ...
  , bundlePosState :: PosState (Tokens s)
    -- ^ State that is used for line\/column calculation
  } deriving (Generic)

deriving instance ( Show (Tokens s)
                  , Show (Element (Tokens s))
                  , Show e
                  ) => Show (ParseErrorBundle s e)

deriving instance ( Eq (Tokens s)
                  , Eq (Element (Tokens s))
                  , Eq e
                  ) => Eq (ParseErrorBundle s e)

deriving instance ( Typeable (Tokens s)
                  , Typeable (Element (Tokens s))
                  , Typeable e
...
instance ( NFData (Tokens s)
         , NFData (Element (Tokens s))
         , NFData e
...
attachSourcePos
  :: forall s a t . (Traversable t, Stream s)
  => (a -> Int) -- ^ How to project offset from an item (e.g. 'errorOffset')
  -> t a               -- ^ The collection of items
  -> PosState (Tokens s)        -- ^ Initial 'PosState'
  -> (t (a, SourcePos), PosState (Tokens s)) -- ^ The collection with 'SourcePos'es
                                    -- added and the final 'PosState'
attachSourcePos projectOffset xs = runState (traverse f xs)
  where
    f a = do
      pst <- get
      let (spos, pst') = nreachOffsetNoLine' @s (projectOffset a) pst
      put pst'
      return (a, spos)
...
errorBundlePretty ParseErrorBundle {..} =
  ...
  in drop 1 (r "")
  where
    f :: Stream s => (ShowS, PosState (Tokens s))
      -> ParseError s e
      -> (ShowS, PosState (Tokens s))
    f (o, !pst) e = (o . (outChunk ++), pst')
      where
        (epos, sline, pst') = nreachOffset @s (errorOffset e) pst
        outChunk =
...
parseErrorTextPretty (TrivialError _ us ps) =
  if isNothing us && E.null ps
    ...
    else messageItemsPretty "unexpected " (showErrorItem @s `E.map` maybe E.empty E.singleton us) <>
         messageItemsPretty "expecting "  (showErrorItem @s `E.map` ps)
...
showErrorItem :: forall s . Stream s => ErrorItem (Element (Tokens s)) -> String
showErrorItem = \case
    Tokens   ts -> prettyTokens @s ts
