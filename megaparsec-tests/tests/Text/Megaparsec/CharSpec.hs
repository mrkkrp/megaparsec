...
spec = do
  ...
  describe "charCategory" $ do
    ...
    context "when parser's category does not match next character's category"
      $ it "fails"
      $ property
      $ \g ch s -> (generalCategory ch /= g) ==> do
        let s' = ch : s
        prs (charCategory g) s'
          `shouldFailWith` err 0 (utok ch <> elabel (categoryName g))
    ...
    context "when stream is empty"
      ...
      $ \g ->
        prs (charCategory g) ""
          `shouldFailWith` err 0 (ueof <> elabel (categoryName g))
  ...
  describe "string'" $ do
    ...
    context "when stream is not prefixed with given string"
      $ it "signals correct parse error"
      $ property
      $ \str s -> not (str `isPrefixOfI` s) ==> do
        let us = take (length str) s
        prs (string' str) s `shouldFailWith` err 0 (utoks us <> etoks str)
